...
const CreateCurso = () => {
    ...
    const uploadImage = async (e) => {
        ...
        await CursosService.uploadCursos(formData).then((response) => {
            setStatus({
                'type': 'success',
                ...
                <div className="create-single">
                    <form onSubmit={uploadImage}>
                        <div className="wraper-create">
                            { image ? <label style={{backgroundImage: "url(" + URL.createObjectURL(image) + ")"}} className="img-label" htmlFor="image"></label> : <label className="label-default" htmlFor="image"><FontAwesomeIcon icon={ faImages }/></label>}
